...
localStorage.setItem(
 ...
 function downloadImprovedResume() {
  ...
  try {
    const bullets = Array.isArray(report.rewrittenBullets)
      ? report.rewrittenBullets
      : [];

    const skills = Array.isArray(report.optimizedSkills)
      ? report.optimizedSkills
      : [];

    const keywords = Array.isArray(report.missingKeywords)
    ...
    const resumeHtml = `
      ...
      <html>
        ...
        <body>
          ...
          <p>${report.improvedSummary || "Summary unavailable."}</p>
          ...
          <ul>
            ${
              bullets.length
                ...
                : "<li>No rewritten bullets available.</li>"
            }
          </ul>
            ...
            ${
              keywords.length
                ? keywords
                    ...
                    .join("")
                : "No additional keywords available."
            }
          </p>
    ...
    const cleanName =
      report.fileName?.replace(/\.[^/.]+$/, "").replace(/[^a-zA-Z0-9_-]/g, "_") ||
      "Improved_Resume";
