...
Best Job Matches:
${report.jobMatches
  .map((job) => `- ${job.company} | ${job.role} | ${job.location} | ${job.match}% match | ${job.url}`)
  .join("\n")}
`;
      ...
      <section className="mx-auto max-w-7xl text-center">
        <div className="mx-auto max-w-5xl">
          ...
          <p className="mx-auto mt-7 max-w-3xl text-lg text-gray-400 md:text-xl">
            ...
            and job matches — with just enough roast to hurt your ego and help your career.
          </p>
        ...
        {report && (
          <div className="mt-8 flex flex-wrap justify-center gap-4">
            ...
            <button
              onClick={() => alert("PDF export is coming next. For now, use Copy Full Report.")}
    ...
    <div className="mx-auto mt-8 max-w-7xl rounded-[2rem] border border-zinc-800 bg-zinc-950 p-6 text-left shadow-2xl md:p-10">
      ...
      <div className="mt-10 rounded-2xl border border-zinc-800 bg-black p-6 text-center">
        ...
        <p className="mt-2 text-gray-400">
          Built to roast weak resumes, rescue strong careers, and mildly offend bullet points.
        </p>
      </div>
    </div>
  );
...
function GaugeCard({ title, value }: { title: string; value: number }) {
  ...
  const pathColor = score >= 75 ? "#22c55e" : score >= 50 ? "#f97316" : "#ef4444";
  ...
  return (
    <div className="mt-8 rounded-3xl border border-zinc-800 bg-zinc-900 p-8 shadow-xl">
      ...
      <ul className="mt-5 list-disc space-y-3 pl-5 text-gray-300">
        {items.length > 0 ? items.map((item) => <li key={item}>{item}</li>) : <li>No data available.</li>}
      </ul>
    </div>
  );
}
